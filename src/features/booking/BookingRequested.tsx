...
export function BookingRequested() {
  ...
  return <section className="requested-card">
    ...
    {receipt ? <><p>{t('requested.reference')}</p><strong className="request-reference">{receipt.reference}</strong>{receipt.summary && <dl className="request-summary"><div><dt>{t('requested.service')}</dt><dd>{receipt.summary.service}</dd></div><div><dt>{t('requested.place')}</dt><dd>{receipt.summary.place}</dd></div><div><dt>{t('requested.date')}</dt><dd>{receipt.summary.date}</dd></div></dl>}<p>{t('requested.contact', { time: receipt.expectedResponse })}</p></> : <p>{t('requested.missing')}</p>}
    ...
    <div className="requested-actions"><Link className="consumer-button consumer-button--primary" href="/auth/">{t('requested.account')}</Link><Link className="text-link" href="/">{t('common.returnHome')}</Link></div>
    <p className="optional-account-copy">{t('requested.optional')}</p>
  </section>
}
